...
def msgtobinary(msg):
    if type(msg) == str:
        result= ''.join([ format(ord(i), "08b") for i in msg ])  #ord gives ascii code
    ...
    return result #result returns 8 bit string number for each character and return type is string
...
def encryption(plaintext): 
    ...
    cipher=keystream^plaintext #xor operation
    ctext=''
    for c in cipher:
        ctext=ctext+chr(c)     
    return ctext
...
def embed(frame):
    ...
    for i in frame:
        for pixel in i:  #pixel is nparray   
            r, g, b = msgtobinary(pixel)  #r , g , b are strings   &   binary_data is string too
            if index_data < length_data:
                
                pixel[0] = int(r[:-1] + binary_data[index_data], 2)   #adds two string and convert in 
        ...
        for pixel in i:
            ...
            for byte in total_bytes:
                ...
                if decoded_data[-5:] == "*^*^*": 
                    ...
                    print("\nThe Encoded data which is hidden in the Video is : ",final_decoded_msg)
                    return 
